...
void proximity(std::vector<std::vector<float>> points, std::vector<int>& cluster, int index, std::vector<bool>& flags, KdTree* tree, float distanceTol){
    flags[index] = true;
    cluster.push_back(index);
    std::vector<int> nearby_pts = tree->search(points[index], distanceTol);
    
    for(int ind : nearby_pts){
        if(!flags[ind])
            proximity(points, cluster, ind, flags, tree, distanceTol);
    }
}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
    ...
    std::vector<bool> flags(points.size(), false);
	std::vector<std::vector<int>> clusters;
    
    for(int i =0; i < points.size(); i++){
        //check if pt is not inside clusters (or) processed
        if (flags[i]){
            continue;
        }
        
        std::vector<int> cluster;
        proximity(points, cluster, i, flags, tree, distanceTol);
